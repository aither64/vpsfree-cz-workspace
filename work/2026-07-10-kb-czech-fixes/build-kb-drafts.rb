...
REPLACEMENTS.each do |page, replacements|
  ...
  media_by_page[page].each do |asset|
    ...
    text = text.gsub(/\{\{[^}]+\}\}/) do |media_ref|
      ...
      replaced += 1
      media_ref.sub(target, ":#{asset.fetch('draft_media')}")
    end

    abort "#{page}: did not replace media reference #{legacy}" if replaced.zero?
  end

  text = text.sub(/\A<page>[^\n]*<\/page>\n/, '')
  revision = metadata.fetch(page).fetch('revision')
  note = <<~NOTE
    <note warning>**Pracovní návrh k revizi.** Zdrojová stránka [[:#{page}|#{page}]], zdrojová revize ''#{revision}''. Tento návrh není publikovaná dokumentace.</note>

  NOTE
  text = "#{note}#{text}"

  preview_path = File.join(ROOT, 'kb-previews', *page.split(':')) + '.txt'
  ...
  drafts << {
    ...
    'draft_page' => "#{DRAFT_NAMESPACE}:#{page}",
    'preview_file' => preview_path.delete_prefix("#{ROOT}/"),
    'screenshot_count' => media_by_page[page].length
  }
end

File.write(
  File.join(ROOT, 'draft-pages.yml'),
  YAML.dump('schema' => 1, 'wiki' => 'kb.vpsfree.cz', 'pages' => drafts)
)

puts "wrote #{drafts.length} draft previews with #{drafts.sum { |d| d.fetch('screenshot_count') }} media references"
