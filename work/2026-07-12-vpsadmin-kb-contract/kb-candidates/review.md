...
| --- | --- | --- | ---: | --- | --- |
| cs | informace:platby | `member.payment-instructions.open` | 1 | **Upravit profil** -> **Pokyny k platbě** | **Upravit profil** -> **Pokyny k platbě** |
| cs | navody:server:ssh | `member.public-keys.add` | 1 | Upravit profil → Veřejné klíče → vpravo v panelu Přidat veřejný klíč | Upravit profil → Veřejné klíče → vpravo v panelu Přidat veřejný klíč |
| cs | navody:vps:sprava | `member.public-keys.add` | 1 | Upravit profil → Veřejné klíče → vpravo v panelu Přidat veřejný klíč | Upravit profil → Veřejné klíče → vpravo v panelu Přidat veřejný klíč |
| cs | navody:vps:metriky | `member.metrics-access-tokens.open` | 1 | **Upravit profil** -> **Přístupové tokeny k metrikám** | **Upravit profil** -> **Přístupové tokeny k metrikám** |
| cs | navody:vps:prostredi | `member.cluster-resources.open` | 1 | Upravit profil → ↵ Prostředky clusteru | Upravit profil → ↵ Prostředky clusteru |
| cs | navody:vps:prostredi | `member.environment-configs.open` | 1 | Upravit profil → Konfigurace prostředí | Upravit profil → Konfigurace prostředí |
| cs | navody:vps:uzivatele | `member.advanced-email-configuration.open` | 1 | vpsAdmin -> Upravit profil -> Pokročilá konfigurace e-mailu | vpsAdmin -> Upravit profil -> Pokročilá konfigurace e-mailu |
...
| cs | navody:server:primarni_dns | `dns.primary-zones.open` | 1 | **DNS** -> **Primární zóny** | **DNS** -> **Primární zóny** |
| cs | navody:server:sekundarni_dns | `dns.secondary-zones.open` | 1 | DNS -> Sekundární zóny | DNS -> Sekundární zóny |
| cs | navody:server:sekundarni_dns | `dns.tsig-keys.open` | 1 | DNS -> TSIG klíče | DNS -> TSIG klíče |
| cs | navody:vps:zalohy | `backups.vps.open` | 2 | menu Zálohy | menu Zálohy → Zálohy VPS |
| cs | navody:vps:obnova_webu_zo_zalohy | `backups.vps.open` | 1 | //Zálohy > Zálohy VPS// | //Zálohy > Zálohy VPS// |
| cs | navody:vps:playgroundvps | `vps.create.open` | 1 | „Nové VPS“ | „Nové VPS“ |
| cs | navody:vps:obnova_webu_zo_zalohy | `vps.create.open` | 1 | //VPS > Nové VPS// | //VPS > Nové VPS// |
| cs | navody:vps:playgroundvps | `vps.clone.open` | 1 | **Klonovat VPS** | **Klonovat VPS** |
...
| en | manuals:vps:environment | `member.environment-configs.open` | 1 | Edit profile → Environment ↵ configs | Edit profile → Environment ↵ configs |
| en | manuals:vps:users | `member.advanced-email-configuration.open` | 1 | vpsAdmin -> Edit profile -> ↵ Advanced e-mail configuration | vpsAdmin -> Edit profile -> ↵ Advanced e-mail configuration |
...
| en | manuals:server:primary_dns | `dns.primary-zones.open` | 1 | **DNS** -> **Primary Zones** | **DNS** -> **Primary zones** |
| en | manuals:server:secondary_dns | `dns.secondary-zones.open` | 1 | DNS -> Secondary Zones | DNS -> Secondary zones |
| en | manuals:server:secondary_dns | `dns.tsig-keys.open` | 1 | DNS -> TSIG Keys | DNS -> TSIG keys |
| en | manuals:vps:backups | `backups.vps.open` | 2 | the Backups menu | the Backups -> VPS backups menu |
| en | manuals:vps:playgroundvps | `vps.create.open` | 1 | section VPS, "New VPS" | section VPS, "New VPS" |
...
| --- | --- | --- | --- |
| en | manuals:vps:management | `vps.create.open` | The article describes creation steps but does not name the New VPS navigation control. |
| en | manuals:vps:management | `vps.clone.open` | The article does not contain a Clone VPS navigation instruction. |
| en | manuals:vps:datasets | `datasets.create.open` | The article discusses subdatasets but does not name the Create dataset control. |
